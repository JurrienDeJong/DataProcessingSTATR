    ...
    for option, arg in options:
        if len(options) == 1:
            if option == '-h':
                ...
                print(
                    'Usage: GenerateProfile.py -i <input BED file> -e <end, either 5 or 3> -n <normalization factor> -o <output file>')
                sys.exit()
            else:
                print('Error while reading arguments. GenerateProfile.py -h')
                sys.exit(2)
                ...
                with open(outfile, 'w') as temp:
                    for pos, score in profile.items():
                        ...
                        line = prefix + [temp_dic[strand], pos[1:], pos[1:],
                                         strand + str(round(float(score) / nfactor, 2)), strand] + suffix
                        temp.write('\t'.join(line))
            except:
                print('No such file or directory: ' + outfile)
                sys.exit(2)
